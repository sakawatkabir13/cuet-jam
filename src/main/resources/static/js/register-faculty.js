// Faculty registration page functionality
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    facultyRegisterForm.addEventListener('submit', async function(e) {
        ...
        clearFormErrors('facultyRegisterForm');
        hideAlert('alert');
        ...
        // Validate form
        if (!validateFacultyForm(formData)) {
            setButtonLoading('registerBtn', false);
            return;
        }
        
        try {
            const response = await API.Auth.registerFaculty({
                name: formData.name,
    ...
    document.getElementById('email').addEventListener('blur', function(e) {
        ...
        if (email && !isValidCUETFacultyEmail(email)) {
            showFieldError('email', 'Please enter a valid CUET faculty email (format: name@cuet.ac.bd or name1234@cuet.ac.bd)');
        } else if (email && isValidCUETFacultyEmail(email)) {
...
function validateFacultyForm(data) {
    ...
    if (!data.email) {
        ...
        isValid = false;
    } else if (!isValidCUETFacultyEmail(data.email)) {
        showFieldError('email', 'Please enter a valid CUET faculty email (format: name@cuet.ac.bd or name1234@cuet.ac.bd)');
        isValid = false;
    }
    ...
    if (data.designation && data.designation.length > 100) {
        showFieldError('designation', 'Designation must not exceed 100 characters');
        isValid = false;
    }
